...
| **Software del Microcontrolador** | Firmware de referencia en `firmware/chess_arm_controller/` (Arduino IDE / `arduino-cli` + librería "Adafruit PWM Servo Driver Library") |

> Arquitectura confirmada e implementada en M8 (`chess_actuators`):
> protocolo Serial/USB entre el Host y el Arduino UNO, que a su vez
> controla el PCA9685 vía I2C — ver `M8_SPEC.md` §2.1 y §3.
...
| **Instrumento de Medición**           | Calibre digital o regla milimetrada — requerido por el protocolo de calibración manual de M0 (ver `M0_SPEC.md` §4) y para medir la pieza más alta del set (insumo de `SAFE_TRAVEL_HEIGHT_MM` en `chess_kinematics`, M7, ver `M7_SPEC.md` §7). Se suma un transportador/goniómetro, requerido por el protocolo de calibración física de servos de `chess_actuators` (M8, ver `M8_SPEC.md` §6). |
